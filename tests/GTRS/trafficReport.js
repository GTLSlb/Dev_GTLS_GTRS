...
describe("Navigation Test", () => {
    ...
    it("user can view the Traffic Report", async () => {
        // Step 1: Navigate to the main page
        await driver.sleep(3000);

        // Step 2: Verify that Traffic Report is displayed
        ...
        assert.strictEqual(
            await title.getText(),
            "Traffic Report",
            "Traffic Report should be displayed."
        );
    });

    for (const { pageName, url } of gtrsPages) {
        it("user can navigate from other pages to the Traffic Report", async () => {
            // Step 1: Navigate to the main page
            await driver.sleep(3000);
            ...
            for (let item of menuItems) {
                ...
                if (text == "Traffic Report") {
                    dashItem = item;
                }
            }

            // Step 3: Navigate to the Traffic Report
            ...
            assert.strictEqual(
                currentUrl,
                "https://gtrs.gtls.store/gtrs/traffic-report",
                `Expected URL to be 'https://gtrs.gtls.store/gtrs/traffic-report' but got '${currentUrl}'.`
            );
        });
    }
});
...
describe("Table Test", () => {
    ...
    it(`Traffic Report data is correct`, async () => {
        try {
            // Step 1: Navigate to the Traffic Report page
            await driver.sleep(2000);
            navigateToPage(driver, "Traffic Report");
            ...
            const resData = await axios.get(
                `${process.env.GTRS_WEB_URL}get-eventsCategories`,
                {
                    headers: {
            ...
            const apiValues = data.map((trafficRep) => {
                return [
                    trafficRep.Name,
                    trafficRep.DeviceCode,
                    trafficRep.SmartSCANSoftwareVersion,
                    trafficRep.Description,
                    trafficRep.LastActiveUTC,
                    trafficRep.SoftwareVersion,
                    trafficRep.MobilityDeviceSimTypes_Description,
                    trafficRep.MobilityDeviceModels_Description,
                    trafficRep.MobilityDeviceMakes_Description,
                ];
            });
    ...
    it("user can clear all filters", async () => {
        try {
            // Step 1: Navigate to the Traffic Report page
            await driver.sleep(2000);
            await navigateToPage(driver, "Traffic Report");
...
describe("Table Test", () => {
    ...
    it("user can view details", async () => {
        // Step 1: Navigate to the main page
        const scrollArea = await driver.findElement(
            By.css(".InovuaReactDataGrid__virtual-list")
        );
        ...
        const actions = driver.findElements(By.xpath('//*[@id="app"]/div/div/div/div/div[2]/div/div/div/div/main/div[2]/div/div/div/div/div/div[1]/div[1]/div[2]/div[1]/div/div[1]/div/div/div[1]/div[1]/div/div[14]/div/span'));
    });
});
