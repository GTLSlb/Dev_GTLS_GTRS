...
export default function Utilization() {
    ...
    const loadData = (dataArray, offset, windowSize) => {
        const res = [];
        if (offset + windowSize > dataArray.length) return "EOF";
        const size = Math.min(offset + windowSize, dataArray.length);
    ...
    useEffect(() => {
        ...
        if (utilizationData) {
            // const initialData = loadData(utilizationData, 0, bufferSize);
            // if (initialData !== "EOF") {
            //     setTableData(initialData[1]);
            // }
    ...
    const renderTable = useMemo(() => {
        ...
        const handleScroll = () => {
            ...
            const last = hotInstance
                .getPlugin("AutoRowSize")
                .getLastVisibleRow();

            // Check if we need to load more data when scrolling forward
            if (last >= tableData.length - 1) {
                // Use a functional update to get the latest offset
                setOffset((prevOffset) => {
                    const newData = loadData(
                        utilizationData,
                        prevOffset + bufferSize,
                        bufferSize
                    );
                    if (newData === "EOF") {
                        return prevOffset;
                    ...
                    // Append the new data to the existing tableData
                    setTableData((currentData) => [
                        ...currentData,
                        ...newData[1],
                    ]);
                    return prevOffset + bufferSize;
                });
            }
        };
    ...
    return (
        <div className="min-h-full px-8">
            ...
            {tableData && !isLoading && renderTable()}
